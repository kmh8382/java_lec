...
public class TryCatchEx {
  ...
  public static void b() {
    ...
    try {
      ...
      String[] userUnputs = {"10", "20", "삼십", "40"};

      for(int i = 0; i <= userUnputs.length; i++) {
        int number = Integer.parseInt(userUnputs[i]);
        System.out.println(number);
      }
      ...
      System.out.println("사용할 수 없는 인텍스가 존재합니다.");
    }
  ...
  public static void c() {
    try {
      ...
      // 예외메시지 : 예외가 발생한 이유가 텍슽로 작성되어 있다.
      String message = e.getMessage();
      
      // 예외 클래스 : 예외 글래스가 저장되어 있다.
