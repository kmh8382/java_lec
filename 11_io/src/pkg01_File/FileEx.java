 ...
 *   2) linux   : /
 *   3) mac     : /
 *   4) File.separator 필드를 사용하면 플랫품에 따라서 자동으로 경로 구분자를 넣어 준다.
 * 2. 디렉터리
 *   1) 현재 디렉터리 : .  
 *   2) 상위 디렉터리 : ..  
 *   3) 홈 디렉터리   : ~   - linux
...
public class FileEx {
  ...
  public static void a() {    
    ...
    //File 인스턴스 생성 (디렉터리 경로와 이름을 작성해야 한다)
  ...
  public static void b() {
    // 디렉터리 삭제하기 : 비어 있는 디렉터리만 삭제ㅏㄹ 수 있다.
...
//      builder.append(String.format("%-30s", new SimpleDateFormat("yyyy-MM-dd a h:mm").format(lastModified)));
  ...
  public static void d() {    
    /* D:\\2024\\11\\15 디렉터리 만들기 실습 (2024폴더 아래 11폴더 아래 15폴더) */
...
//    String result = formatter.format(now);
  ...
  public static void e(String pathname) {    
    ...
    if(dir.exists()) {      
      ...
      System.out.println(dir.getPath() + " 디렉터리가 없습니다.");
    }
    
    // 재귀호출 (잘못하면 무한으로 자기자신을 호출하게 된다...)
    e(dir.getParent());
