...
public class OneEx {
  ...
  public static void main(String[] args) {
    ...
    String[] movies;
    movies = new String[] {"타이타닉", "스타워즈", "에일이언"};
    System.out.println(Arrays.toString(movies));
    
    // 배열 길이
    String[] flowers = {"장미", "동잭", "할미"};
    System.out.println(flowers.length);
    // flowers.length = 10;   // 불가능 (length 필드는 final 필드)
...
//    temp[3] = "겨울";
    ...
    seasons = temp;        
    System.out.println(Arrays.toString(seasons));   // seasons에 [봄, 여름, 가을, 겨울]

    // 배열 순회 (Advanced for 문 - 배열 요소를 수정할때는 사용할 수 았다.)
